...
```{r}
# comprobar pacientes seleccionados 

aux <- xlsx::read.xlsx("07_Data/PacientsCIBERPOSTCOVIDesperats_230330.xlsx",sheetIndex = 1)
...
dat$M36_dlco_80 <- relevel(factor(dat$M36_dlco_80),ref = ">80")
dat$M36_dlco_60 <- relevel(factor(dat$M36_dlco_60),ref = ">60")
...
```{r}
DT::datatable(dat %>% dplyr::select(Redcap.Ciberes,IH_hostdat_validated,AH_f_datehosp,M36_followup_spirometry_date,time,time2, M36_followup_dlco), extensions = "Buttons", options = list(pageLength = 60, dom = "Bftip", buttons = c("copy", "csv", "excel", "pdf")))
...
res <- compareGroups( M36_dlco_80~., data = dat  %>% dplyr::select(vec_names_bas,M36_dlco_80),
                      method = NA, 
                      include.miss = FALSE)
...
## Tabla secuelas en funcion de DLCO >80


M36_result_tac_followup___0 = "Normalización radiológica"
M36_result_tac_followup___1 = "Infiltrados persistentes"
M36_result_tac_followup___2 = "EPID"
M36_result_tac_followup___3 = "Tractos fibrosos"
M36_result_tac_followup___4 = "Enfisema"
M36_result_tac_followup___5 = "Otras alteraciones"


```{r}

vec_names_bas <- dat %>%  dplyr::select(M36_dlco_80,M36_followup_dlco, M36_followup_spirometry_fev, M36_followup_spirometry_fvc, M36_followup_spirometry_fev_fvc, M36_result_tac_followup___0,M36_result_tac_followup___1, M36_result_tac_followup___2, M36_result_tac_followup___3, M36_result_tac_followup___5, M36_result_tac_followup___4,  ) %>%  names


res <- compareGroups( M36_dlco_80~., data = dat  %>% dplyr::select(vec_names_bas),
                      method =NA, 
                      include.miss = FALSE)
...
### Differential expression  with adjusted by AGE + SEXO + HTA + PUL + RENAL + OBESITY +  DIABETES
...
res_de <- expresion_diferencial_logbase(datos = dat, var_grupo = "M36_dlco_80", vars_ajuste = c("IH_age_estimateyears","IH_sex","IH_hypertension_mhyn","IH_chronicpul_mhyn","IH_renal_mhyn", "IH_obesity_mhyn", "IH_diabetes_mhyn_2"),  noms_proteines = miRnames,names_id ="REDCap",logbase = 10, pval = 0.05, FC_limit = 1.25)
...
res_de <- expresion_diferencial_logbase(datos = dat, var_grupo = "M36_dlco_80", NULL, miRnames,"REDCap",logbase = 10, pval = 0.05, FC_limit = 1.25)
DE <- res_de[[2]][which((res_de[[2]]$FC>1.25 | res_de[[2]]$FC<0.8) & res_de[[2]]$p.value <0.05),"Names"]
DE_plasma <- DE
...
```{r, comment="",fig.width=20,fig.height=25,eval=TRUE}
## Seleccion de variables 
dat.s <- dat %>% dplyr::select(miRNAs,M36_dlco_80) %>% filter(complete.cases(.))
...
library(VSURF)
set.seed(12)
vsel<-VSURF(dat.s[,1:(dim(dat.s)[2] - 1)], dat.s[,dim(dat.s)[2]],verbose = FALSE)
# triga alguns minuts
...
```{r}
dat[,miRNAs] <- impute.nipals(X = dat[,miRNAs], ncomp = 3)
...
for( i in miRNAs){
  dat$mir1 <- dat[,i]
  mtHL <- maxstat.test(M36_dlco_80 ~ mir1,data= dat , smethod="Wilcoxon", pmethod="HL",minprop = 0.10)
...
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears,IH_sex , IH_smoking_mhyn,  AH_noninvaisve_prdur,  AH_invasive_prdur, AH_ef_infyn, miR.93.5p.cat,miR.199a.5p.cat,miR.27a.3p.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  IH_age_estimateyears + IH_sex + IH_smoking_mhyn + AH_noninvaisve_prdur + AH_invasive_prdur + AH_ef_infyn + ",paste0(c("miR.93.5p.cat","miR.199a.5p.cat","miR.27a.3p.cat"), collapse = "+"))) ,
...
auc1 <-auc(roc(fit$data$M36_dlco_80,test_pred))
auc2 <- auc(roc(f1$data$M36_dlco_80,test_pred2))
# compare concordance values of fit4 and fit5
roc.test(roc(fit$data$M36_dlco_80,test_pred),roc(f1$data$M36_dlco_80,test_pred2))

test_pred <- predict(fit, type = "response")
test_pred2 <- predict(f1, type = "response")

dat.s$M36_dlco_80.f <- ifelse(dat.s$M36_dlco_80 == ">80",0,1)
...
dat.s <- dat %>% dplyr::select(M36_dlco_60,IH_age_estimateyears,IH_chronicpul_mhyn, AH_invasive_prdur, IU_urea,IU_filtrado, miR.93.5p.cat,miR.199a.5p.cat,miR.27a.3p.cat,time2) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~ IH_age_estimateyears+IH_chronicpul_mhyn+ AH_invasive_prdur+ IU_urea + IU_filtrado  +",paste0(c("miR.93.5p.cat","miR.199a.5p.cat","miR.27a.3p.cat"), collapse = "+"))) ,
              lower = ~   IH_age_estimateyears+IH_chronicpul_mhyn+ AH_invasive_prdur+ IU_urea + IU_filtrado  )

f1 <- glm(M36_dlco_60 ~  IH_age_estimateyears+IH_chronicpul_mhyn+ AH_invasive_prdur+ IU_urea + IU_filtrado ,data = dat.s,family = "binomial")
fit <- step(f1, scope = scope,scale = TRUE,trace = FALSE)
...
auc1 <-auc(roc(fit$data$M36_dlco_60,test_pred))
auc2 <- auc(roc(f1$data$M36_dlco_60,test_pred2))
# compare concordance values of fit4 and fit5
roc.test(roc(fit$data$M36_dlco_60,test_pred),roc(f1$data$M36_dlco_60,test_pred2))

test_pred <- predict(fit, type = "response")
test_pred2 <- predict(f1, type = "response")

dat.s$M36_dlco_60.f <- ifelse(dat.s$M36_dlco_60 == ">60",0,1)
PredictABEL::reclassification(data = dat.s, 
                              cOutcome = 11, 
                              predrisk1 = test_pred2, # original model
                              predrisk2 = test_pred, # updated model
...
```{r}
# continua
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears, AH_invasive_prdur,IU_filtrado,miR.93.5p.cat,miR.199a.5p.cat,miR.27a.3p.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  IH_age_estimateyears + AH_invasive_prdur + IU_filtrado +",paste0(c("miR.93.5p.cat","miR.199a.5p.cat","miR.27a.3p.cat"), collapse = "+"))) ,
              lower = ~   IH_age_estimateyears + AH_invasive_prdur + IU_filtrado)

f1 <- glm(M36_dlco_80 ~   IH_age_estimateyears + AH_invasive_prdur + IU_filtrado  ,data = dat.s, family = "binomial")
fit <- step(f1, scope = scope,scale = TRUE,trace = FALSE)
...
dat$M36_result_tac_followup___3 <- factor(ifelse(dat$M36_result_tac_followup___3 == 2,"No","Yes"))
# Fibrosis 

dat.s <- dat %>% dplyr::select(M36_result_tac_followup___3,time2, AH_bactpneu_ceterm,  AH_noninvaisve_prdur,  AH_invasive_prdur, miR.93.5p.cat,miR.199a.5p.cat,miR.27a.3p.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur + ",paste0(c("miR.93.5p.cat","miR.199a.5p.cat","miR.27a.3p.cat"), collapse = "+"))) ,
              lower = ~    AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur)

f1 <- glm(I(M36_result_tac_followup___3 == "Yes") ~  AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur ,data = dat.s,family = "binomial")
fit <- step(f1, scope = scope,scale = TRUE,trace = FALSE)
...
auc1 <-auc(roc(fit$data$M36_result_tac_followup___3,test_pred))
auc2 <- auc(roc(f1$data$M36_result_tac_followup___3,test_pred2))
# compare concordance values of fit4 and fit5
roc.test(roc(fit$data$M36_result_tac_followup___3,test_pred),roc(f1$data$M36_result_tac_followup___3,test_pred2))

test_pred <- predict(fit, type = "response")
test_pred2 <- predict(f1, type = "response")

dat.s$M36_result_tac_followup___3.f <- ifelse(dat.s$M36_result_tac_followup___3 == "No",0,1)
PredictABEL::reclassification(data = dat.s, 
                              cOutcome = 9, 
                              predrisk1 = test_pred2, # original model
                              predrisk2 = test_pred, # updated model
...
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears,IH_sex , IH_smoking_mhyn,  AH_noninvaisve_prdur,  AH_invasive_prdur, AH_ef_infyn, miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 <6)
set.seed(125)

f1 <- glm(I(M36_dlco_80 == "<80") ~ IH_age_estimateyears + 
    IH_sex + IH_smoking_mhyn + AH_noninvaisve_prdur + AH_invasive_prdur + 
    AH_ef_infyn ,data = dat.s,family = "binomial")
fit <- glm(I(M36_dlco_80 == "<80") ~ IH_age_estimateyears + 
    IH_sex + IH_smoking_mhyn + AH_noninvaisve_prdur + AH_invasive_prdur + 
    AH_ef_infyn + miR.199a.5p.cat + miR.93.5p.cat,data = dat.s,family = "binomial")
...
dat.s <- dat %>% dplyr::select(M36_dlco_60,IH_age_estimateyears,IH_chronicpul_mhyn, AH_invasive_prdur, IU_urea,IU_filtrado, miR.93.5p.cat,miR.199a.5p.cat,time2) %>% filter(complete.cases(.)) %>% filter(time2 <6)
set.seed(125)

f1 <- glm(M36_dlco_60 ~ IH_age_estimateyears + IH_chronicpul_mhyn + 
    AH_invasive_prdur + IU_urea + IU_filtrado,data = dat.s,family = "binomial")
fit <- glm(M36_dlco_60 ~ IH_age_estimateyears + IH_chronicpul_mhyn + 
    AH_invasive_prdur + IU_urea + IU_filtrado,data = dat.s,family = "binomial")
...
```{r}
# continua
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears, AH_invasive_prdur,IU_filtrado,miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 <6)
set.seed(125)
f1 <- glm(M36_dlco_80 ~ IH_age_estimateyears + AH_invasive_prdur + 
    IU_filtrado  ,data = dat.s, family = "binomial")
fit <- glm(M36_dlco_80 ~ IH_age_estimateyears + AH_invasive_prdur + 
    IU_filtrado + miR.199a.5p.cat + miR.93.5p.cat  ,data = dat.s, family = "binomial")
...
dat.s <- dat %>% dplyr::select(M36_result_tac_followup___3,time2, AH_bactpneu_ceterm,  AH_noninvaisve_prdur,  AH_invasive_prdur, miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 <6)
set.seed(125)

f1 <- glm(I(M36_result_tac_followup___3 == "Yes") ~ AH_bactpneu_ceterm + 
    AH_noninvaisve_prdur + AH_invasive_prdur ,data = dat.s,family = "binomial")
fit <- glm(I(M36_result_tac_followup___3 == "Yes") ~ AH_bactpneu_ceterm + 
    AH_noninvaisve_prdur + AH_invasive_prdur + miR.93.5p.cat + 
    miR.199a.5p.cat ,data = dat.s,family = "binomial")
...
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears,IH_sex , IH_smoking_mhyn,  AH_noninvaisve_prdur,  AH_invasive_prdur, AH_ef_infyn, miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 >=6 & time2<12)
set.seed(125)
...
dat.s <- dat %>% dplyr::select(M36_dlco_60,IH_age_estimateyears,IH_chronicpul_mhyn, AH_invasive_prdur, IU_urea,IU_filtrado, miR.93.5p.cat,miR.199a.5p.cat,time2) %>% filter(complete.cases(.)) %>% filter(time2 >=6 & time2<12)
set.seed(125)
...
```{r}
# continua
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears, AH_invasive_prdur,IU_filtrado,miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 >=6 & time2<12)
set.seed(125)
f1 <- glm(M36_dlco_80 ~ IH_age_estimateyears + AH_invasive_prdur + 
    IU_filtrado  ,data = dat.s, family = "binomial")
fit <- glm(M36_dlco_80 ~ IH_age_estimateyears + AH_invasive_prdur + 
...
dat.s <- dat %>% dplyr::select(M36_result_tac_followup___3,time2, AH_bactpneu_ceterm,  AH_noninvaisve_prdur,  AH_invasive_prdur, miR.93.5p.cat,miR.199a.5p.cat) %>% filter(complete.cases(.)) %>% filter(time2 >=6 & time2<12)
set.seed(125)
scope <- list(upper =  formula(paste0("~  AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur + ",paste0(c("miR.93.5p.cat","miR.199a.5p.cat"), collapse = "+"))) ,
              lower = ~    AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur)
...
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears,IH_sex , IH_smoking_mhyn,  AH_noninvaisve_prdur,  AH_invasive_prdur, AH_ef_infyn, miR.16.5p.cat,miR.192.5p.cat,miR.323a.3p.cat,miR.451a.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  IH_age_estimateyears + IH_sex + IH_smoking_mhyn + AH_noninvaisve_prdur + AH_invasive_prdur + AH_ef_infyn + ",paste0(c("miR.16.5p.cat","miR.192.5p.cat","miR.323a.3p.cat","miR.451a.cat"), collapse = "+"))) ,
              lower = ~    IH_age_estimateyears + IH_sex + IH_smoking_mhyn + AH_noninvaisve_prdur + AH_invasive_prdur + AH_ef_infyn  )
...
dat.s <- dat %>% dplyr::select(M36_dlco_60,IH_age_estimateyears,IH_chronicpul_mhyn, AH_invasive_prdur, IU_urea,IU_filtrado, miR.16.5p.cat,miR.192.5p.cat,miR.323a.3p.cat,miR.451a.cat,time2) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~ IH_age_estimateyears+IH_chronicpul_mhyn+ AH_invasive_prdur+ IU_urea + IU_filtrado  +",paste0(c("miR.16.5p.cat","miR.192.5p.cat","miR.323a.3p.cat","miR.451a.cat"), collapse = "+"))) ,
              lower = ~   IH_age_estimateyears+IH_chronicpul_mhyn+ AH_invasive_prdur+ IU_urea + IU_filtrado  )
...
```{r}
# continua
dat.s <- dat %>% dplyr::select(M36_dlco_80,time2, IH_age_estimateyears, AH_invasive_prdur,IU_filtrado,miR.16.5p.cat,miR.192.5p.cat,miR.323a.3p.cat,miR.451a.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  IH_age_estimateyears + AH_invasive_prdur + IU_filtrado +",paste0(c("miR.16.5p.cat","miR.192.5p.cat","miR.323a.3p.cat","miR.451a.cat"), collapse = "+"))) ,
              lower = ~   IH_age_estimateyears + AH_invasive_prdur + IU_filtrado)
...
dat.s <- dat %>% dplyr::select(M36_result_tac_followup___3,time2, AH_bactpneu_ceterm,  AH_noninvaisve_prdur,  AH_invasive_prdur, miR.16.5p.cat,miR.192.5p.cat,miR.323a.3p.cat,miR.451a.cat) %>% filter(complete.cases(.))
set.seed(125)
scope <- list(upper =  formula(paste0("~  AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur + ",paste0(c("miR.16.5p.cat","miR.192.5p.cat","miR.323a.3p.cat","miR.451a.cat"), collapse = "+"))) ,
              lower = ~    AH_bactpneu_ceterm + AH_noninvaisve_prdur + AH_invasive_prdur)
