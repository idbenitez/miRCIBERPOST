...
## read miRNAs
miR <- xlsx::read.xlsx("07_Data/Datos para Iván MM3 CIBERPOSTCOVID Fase 2 DdGC 230214.xlsx",sheetIndex = 1)
miR <- miR  %>%  dplyr::filter( is.na(miR$Comment)) %>% dplyr::select(-Comments)

miR <- miR  %>%  tidyr::spread(miRNA.ID, log2.dct) 
...
aux2 <- aux %>% 
  group_by(IH_subjid) %>% 
  mutate( dif = abs(min(aux$time) - time)) %>% 
  slice(which.min(dif)) %>% 
...
```{r}
DT::datatable(dat %>% dplyr::select(REDCap,`hsa-miR-132-3p`:`hsa-miR-98-5p`), extensions = "Buttons", options = list(pageLength = 20, dom = "Bftip", buttons = c("copy", "csv", "excel", "pdf")))
```
...
labels = FALSE
    ...
    dat$Group <- factor(dat$M36_dlco_80)
    groups <-  dat$Group
    names <- dat$REDCap
    to_outliers <- dat %>% dplyr::select(`hsa-miR-132-3p`:`hsa-miR-98-5p`) 
...
vec_names_bas <- dat %>%  dplyr::select(M36_dlco_trend,`hsa-miR-132-3p`:`hsa-miR-98-5p`) %>%  names
...
miRnames <- dat %>% dplyr::select(`hsa-miR-132-3p`:`hsa-miR-98-5p`) %>% names()
...
# }
aux <- t(dat %>% dplyr::select(`hsa-miR-132-3p`:`hsa-miR-98-5p`))

rownames(aux) <- colnames(dat %>% dplyr::select(`hsa-miR-132-3p`:`hsa-miR-98-5p`))
...
aux <- data.frame(dat %>% dplyr::select(`hsa-miR-132-3p`:`hsa-miR-98-5p`))
...
aux2 <- dat %>% dplyr::select(M36_dlco_80,`hsa-miR-132-3p`:`hsa-miR-98-5p`)
aux2$M36_dlco_80 <- factor(aux2$M36_dlco_80)
sas=reshape::melt(aux2)
