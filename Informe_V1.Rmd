---
title: "miRNAs CIBERESUCICOVID PostCOVID"
author: "Iván D Benítez"
date: "`r Sys.Date()`"
...
# Creacion de visita short term (3-6 meses)

Dado que hay pacientes que disponen de información en la visita 6 meses y otros en la visista 3 meses, se decide crear una visita "short term" combinando ambas visistas. Se seleccionó la visita disponible o en el caso de disponer ambas, se seleccionó la más cercana a la mediana de seguimiento de las visitas 3-6 meses. 

**Aquellos pacientes con tiempo trancurrido hasta la visita "Short-term" fue mayor a 9 meses fueron excluidos**
...
dat$included <- ifelse(!is.na(dat$M36_followup_dlco) & !is.na(dat$ID.TRRM) & dat$time>2,"Included","Without followup")
...
# Baseline characteristics of the cohort according to information available in short term visit.
...
## ID sin info en exitus

```{r}
dat$ID.TRRM[is.na(dat$exitus)]
```

## ID sin info en exitus

```{r}
dat <- dat %>%  filter(!(ID.TRRM %in% c("139","147","201","220","224","228","234","433","471","498")))

c("139","147","201","220","224","228","234","433","471","498")
```




## Tiempo desde el ingreso a UCI hasta la extracción de muestra 

```{r}
summary(dat$IU_tiempo_muestra)
plot(dat$IU_tiempo_muestra )
```
...
Pacientes que no cumplen alguna de estas condiciones 

* fecha inici sintomas <= inicio hospital
* Fecha de muestra >= fecha inicio hospital
* Fecha dia 3 UCI > fecha inicio de UCI
* Fecha inicio VMI > fecha inicio UCI
* Fecha fin VMI > fecha inici VMI
* Fecha alta UCI > fecha inicio UCI
* Fecha alta hospital >= fecha alta UCI

```{r}

dat2 <- dat %>% filter(
  (IH_cestdat>IH_hostdat_validated) | 
  (IH_fecha_muestra_rev<IH_hostdat_validated) |
  (IH_hostdat_validated>IH_icu_hostdat_validated) |
  (D3_daily_dsstdat_daily<=IH_icu_hostdat_validated) | 
  (IV_mv_datestart_validated<IH_icu_hostdat_validated) |
  (IV_mv_datestart_validated>FV_fin_vmi_validated) |
  (AU_alta_uci_validated<IH_icu_hostdat_validated) |
  (AU_alta_uci_validated>AH_f_datehosp)) %>% dplyr::select(RedCap,IH_cestdat,IH_hostdat_validated,IH_fecha_muestra_rev,IH_icu_hostdat_validated,D3_daily_dsstdat_daily,IV_mv_datestart_validated,FV_fin_vmi_validated,AU_alta_uci_validated,AH_f_datehosp)

DT::datatable(dat2, extensions = "Buttons", options = list(pageLength = 20, dom = "Bftip", buttons = c("copy", "csv", "excel", "pdf")), caption = "date queries")
```

Hay diferencias entre la fecha de muestra de la base de datos de miRNAs y la de CIBERESUCICOVID

```{r, eval=FALSE}
#dat <- merge(miR, dat,by.x="RedCap",by.y="RedCap",all.y = TRUE)

dat2 <- dat %>% dplyr::filter(IH_fecha_muestra==IH_fecha_muestra_rev) %>% dplyr::select(RedCap,ID.TRRM,IH_fecha_muestra,IH_fecha_muestra_rev)

DT::datatable(dat2, extensions = "Buttons", options = list(pageLength = 20, dom = "Bftip", buttons = c("copy", "csv", "excel", "pdf")), caption = "date queries")
```

Hay diferencias entre la fecha de ingreso a UCI de la base de datos de miRNAs y la de CIBERESUCICOVID

```{r}

dat2 <- dat %>% filter(IH_icu_hostdat_validated!=IH_icu_hostdat_validated) %>% dplyr::select(RedCap,ID.TRRM,IH_icu_hostdat_validated,IH_icu_hostdat_validated)

DT::datatable(dat2, extensions = "Buttons", options = list(pageLength = 20, dom = "Bftip", buttons = c("copy", "csv", "excel", "pdf")), caption = "date queries")
```

Podemos considerar alta hospitalaria en aquellos pacientes que han sido trasladados con intubación?


```{r}
table(dat$AH_outcomes,dat$AH_intub_transf,useNA = "ifany")
```


# Subestudio 1. FASE AGUDA
...
dat$Included <-  ifelse(dat$RedCap %in% miR$RedCap,"Included", "Excluded")

vec_names_bas <- dat %>%  dplyr::select(IH_age_estimateyears, IH_sex, IH_IMC, IH_smoking_mhyn, IH_ia_alcohol, IH_ia_drug_all, 
                                    IH_chroniccard_mhyn:IH_otherrisktext,
                                    IU_APACHE, IU_SOFA, IU_SOFAh, IU_SOFAr,IU_PAFI, IU_ef_resp_daily, IU_ef_ifo2_daily:IU_oxy_vsorres_daily, 
                                    IU_haemoglobin:IU_cardbnp,syn_ICU_time,Hosp_ICU_time,IU_tiempo_muestra,AH_antibiotic_cmyn,AH_antiviral_cmtrt___2,AH_antiviral_cmtrt___7,AH_ef_convales,AH_heparint_cmyn,AH_antiviral_cmtrt___6,AH_corticost_cmyn,AH_highflow_proccur,AH_noninvasive_proccur,AH_invasive_proccur,AH_pronevent_prtrt,AH_extracorp_prdur,AH_f_alive28,AH_f_deathsite,AH_invasive_prdur,AH_hodur,estacia_Hospital,syn_HOSP_time,Hosp_ICU_time,ICU_IMV_time,Included,IH_covid_vaccine) %>%  names

res <- compareGroups( Included~., data = dat %>% dplyr::select(vec_names_bas,exitus),
                      method = NA, 
                      include.miss = FALSE)
tab <- createTable(res,show.all=T, show.n = TRUE, hide.no = "0")
export2md(tab)

export2xls(tab,"09_results/Tabla_comparativa_incluidos.xlsx")
```




## miRNAs que no validan la dirección de la fase discovery
...
```{r,eval=TRUE }
summa.fit <- decideTests(res_de[[4]], adjust.liphod = "none")
counts <- as.matrix(t(dat[, miRnames]))
rownames(counts) <- colnames(dat[,miRnames])
colnames(counts) <- dat$ID
#Glimma::glXYPlot(x=(res_de[[4]]$coefficients[,1]), y=res_de[[4]]$lods[,1],samples = dat$RedCap,
#           xlab="logFC", ylab="B", main="Volcano plot", counts = counts, groups= dat$exitus.f,
#           levels =levels(dat$exitus.f), status=(summa.fit[,1]) , folder="volcano",html= "vol2")
```

Para consultar la tabla de resultados y el volcano plot interactivo [pulse aquí](volcano/vol2.html). 
...
```{r,eval=FALSE }
summa.fit <- decideTests(eBa,adjust.liphod = "none")
counts <- as.matrix(t(dat %>% dplyr::select((miR.132.3p:miR.98.5p))))
rownames(counts) <- colnames(dat %>% dplyr::select((miR.132.3p:miR.98.5p)))
colnames(counts) <- dat$RedCap

# Glimma::glXYPlot(x=(eBa$coefficients[,1]), y=eBa$lods[,1],samples = dat$NHC,
#          xlab="logFC", ylab="B", main="Volcano plot", counts = counts, groups= factor(dat$M36_dlco_80),levels = levels(factor(dat$M36_dlco_80)), status=(summa.fit[,1]) , folder="volcano",html= "vol1")

```

Para consultar la tabla de resultados y el volcano plot interactivo [pulse aquí](volcano/vol1.html). 
