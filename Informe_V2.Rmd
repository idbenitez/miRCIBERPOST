...
dat <- dat %>% filter(IH_subjid %in% check$x)
```
...
dat_aux <- dat %>% filter(!is.na(M36_admission_followup) & !is.na(ID.TRRM) & time>2 & dat$time<13)

dat$included <- ifelse(!is.na(dat$M36_followup_dlco) & !is.na(dat$ID.TRRM) & dat$time>2  & dat$time<13,"Included","Without followup")
...
dat.s$IH_smoking_mhyn <- relevel(dat.s$IH_smoking_mhyn,ref = "No")
set.seed(125)
scope <- list(upper =  formula(paste0("~  scale(IH_age_estimateyears) + IH_sex + IH_smoking_mhyn + IH_hypertension_mhyn + scale(AH_noninvaisve_prdur) + scale(AH_invasive_prdur) + AH_ef_infyn + scale(IU_filtrado) +",paste0(Gene_ID,".cat", collapse = "+"))) ,
              lower = ~  scale(IH_age_estimateyears) + IH_sex + IH_smoking_mhyn + IH_hypertension_mhyn + scale(AH_noninvaisve_prdur) + scale(AH_invasive_prdur) + AH_ef_infyn + scale(IU_filtrado) )

f1 <- glm(M36_dlco_80.f ~  scale(IH_age_estimateyears) + IH_sex + IH_smoking_mhyn + IH_hypertension_mhyn + scale(AH_noninvaisve_prdur) + scale(AH_invasive_prdur) + AH_ef_infyn + scale(IU_filtrado) ,data = dat.s,family = "binomial")
fit <- step(f1, scope = scope,scale = TRUE,trace = FALSE)

as <- as.data.frame(summary(fit)$coefficients)
as$name <- rownames(as)
colnames(as) <- c("beta","se","z","pvalue","name")
as <- as[-1,]
as$name <- c("Age (years)","Sex (female)","Former smoker (yes)", "Smoker (yes)", "Hypertension (yes)", "NIMV duration (days)", "IMV duration (days)", "Infectious complication (yes)", "Glomerular filtration at ICU admission\n(mL/min/1.73m2)", "miR-323a-3p (High values)",  "miR-148a-3p (High values)",  "miR-16-5p (High values)")
...
p2 <- forestplot(
  ...
  fatten = 20,
  xtickbreaks = c(1/10, 1/5, 1/2, 1.0, 2, 5, 10)
)

cairo_pdf("09_results/Figura_poster.pdf",width = 8,height = 8)
p2
dev.off()
...
pred1 <- as.data.frame(predict(mod1,type = c("link")))
pred2 <- as.data.frame(predict(mod2,type = c("link")))



predt <- list(pred1[,1],
                    pred2[,1])
names(predt) <- c("Clinical model","Clinical model + miRNAs")
outt <- list(mod1$y,mod2$y)

x   <- ROCR::prediction(predt, outt)
ROC <- ROCR::performance(x, "tpr", "fpr")
#ROCR::performance(ROCR::prediction(predt,outt), measure = "auc")@y.values
pdf("09_results/AUC.pdf",width = 8,height = 8)
ROCR::plot(ROC, col = as.list(c(4,3)))
legend("bottomright", legend = c("Clinical model (AUC = 0.78)","Clinical model + miRNAs (AUC = 0.89)"), fill = 3:4)
